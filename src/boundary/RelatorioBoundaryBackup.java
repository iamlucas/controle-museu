 ...
 *
 */
public class RelatorioBoundaryBackup implements ActionListener {
	...
	public RelatorioBoundaryBackup() {
	...
	public void init() {
		...
		pnlPrincipal.add(pnlJasper);
		frame.setIconImage(
				Toolkit.getDefaultToolkit().getImage(RelatorioBoundaryBackup.class.getResource("/resource/mini_logo.png")));

		frame.setContentPane(pnlPrincipal);

		JLabel label = new JLabel("");
		label.setBounds(-42, 14, 345, 122);
		label.setIcon(new ImageIcon(RelatorioBoundaryBackup.class.getResource("/resource/Logo Museu.png")));
	...
	public JRViewer relatorioFaixaEtaria() {
		...
		try {
			...
			viewer = this.criaVisualizacaoRelatorio("/reports/VisitanteRelatorio.jrxml",
	...
	public JRViewer criaVisualizacaoRelatorio(String resource, JRResultSetDataSource data) {
		...
		try {
			...
			URL url = RelatorioBoundaryBackup.class.getResource(resource);
			...
			viewer.setBounds(362, 5, 450, 150);
		} catch (JRException e) {
			e.printStackTrace();
		}
		return viewer;
	...
	public static void main(String[] args) {
		new RelatorioBoundaryBackup().relatorioFaixaEtaria();
